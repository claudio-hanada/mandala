<?php
    ...
    if(isset($_POST["submit"])) {
      $check = getimagesize($_SESSION['file']["tmp_name"]);
      if($check !== false) {
    ...
    // if everything is ok, try to upload file
      ...
      if (move_uploaded_file($_SESSION['file']["tmp_name"], $target_file)) {
        echo "The file ". htmlspecialchars( basename( $_SESSION['file']["name"])). " has been uploaded.";
      } else {
        echo "Sorry, there was an error uploading your file.";
      }
    }

    $_foto = $target_dir . $_SESSION['file']['name'];
